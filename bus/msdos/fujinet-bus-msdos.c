...
extern uint32_t fujiF5w(uint16_t descrdir, uint16_t devcom,
		   ...
		   const void *buf, size_t buf_length)
{
  ...
  fuji_bus_call_rlen = rcode >> 16;
  return (rcode & 0xff) == PACKET_COMPLETE;
}

size_t network_bus_read(uint8_t device, void *buffer, size_t length)
{
  if (!NETCALL_B12_RV(FUJICMD_READ, device - FUJI_DEVICEID_NETWORK + 1,
